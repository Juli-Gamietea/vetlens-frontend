...
export const Diagnosis = ({ route, navigation }) => {
  ...
  React.useEffect(() => {
    diagnosis.anamnesis.inferences.forEach((element) => {
      ...
      if (
        compareDiseaseName(element.disease.name, diagnosis.anamnesis.result)
      ) {
        setProbability(
          (parseFloat(element.probability.replace(",", ".")) * 100).toFixed(2)
        );
        setTreatments(element.disease.treatments);
        setDisease(element.disease.name);
      }
    });
  }, []);
  ...
  const [notDiscernible, setNotDiscernible] = React.useState(false);
    ...
    <ScrollView style={styles.container}>
      ...
      <View style={styles.dogInfoContainer}>
        <View style={styles.dogPhotoContainer}>
          ...
          <Image
            source={{ uri: diagnosis.dog.photoUrl }}
          ...
          <View style={styles.bioColumn}>
            <View style={{ flexDirection: "row" }}>
              ...
              <Text style={styles.textInfo}>
                {" "}
                {diagnosis.dog.sex === "MALE" ? "Macho" : "Hembra"}
              </Text>
            </View>
            <View style={{ flexDirection: "row" }}>
            ...
            <View style={{ flexDirection: "row" }}>
              ...
              <Text style={styles.textInfo}>
                {" "}
                {getAge(diagnosis.dog.date_of_birth)}
              </Text>
            </View>
            <View style={{ flexDirection: "row" }}>
              ...
              <Text style={styles.textInfo}>
                {" "}
                {diagnosis.dog.is_castrated ? "Si" : "No"}
              </Text>
            </View>
          </View>
          <View style={styles.bioColumn}>
        ...
        <View style={styles.resultContainer}>
          ...
          {notDiscernible ? (
            <Text style={styles.resultText}>
              VetLens no pudo determinar con exactitud la enfermedad, puede
              tratarse de una enfermedad fuera de nuestro alcance por el
              momento.
            </Text>
          ) : (
            <Text style={styles.resultText}>
              Según VetLens, {diagnosis.dog.name} padece de "{disease}" con un{" "}
              {probability}% de confiabilidad
            </Text>
          )}
        </View>
      )}
      ...
      >
        <View>
          <Text style={styles.buttonTitle}>
            {role === "VET" ? "Validar" : "Generar código QR"}
          </Text>
        </View>
        <View style={{ flexDirection: "row", alignItems: "center" }}>
          <FontAwesome name="chevron-right" size={30} color="#00767D" />
